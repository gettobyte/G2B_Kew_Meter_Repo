...
//Initialize server.
void svr_init(dlmsServerSettings *settings,
		unsigned char useLogicalNameReferencing,
		DLMS_INTERFACE_TYPE interfaceType, uint16_t frameSize, uint16_t pduSize,
		unsigned char *frameBuffer, uint16_t frameBufferSize,
		unsigned char *pduBuffer, uint16_t pduBufferSize) {
	cl_init(&settings->base, useLogicalNameReferencing, 0, 0,
			DLMS_AUTHENTICATION_NONE, NULL, interfaceType);
...
//Initialize client.
void cl_init(dlmsSettings *settings, unsigned char useLogicalNameReferencing,
		uint16_t clientAddress, uint32_t serverAddress,
		DLMS_AUTHENTICATION authentication, const char *password,
	...
	if (useLogicalNameReferencing) {
		...
		settings->proposedConformance =
				(DLMS_CONFORMANCE) (DLMS_CONFORMANCE_INFORMATION_REPORT
...
void cl_clear(dlmsSettings *settings) {
	settings->protocolVersion = 0;
#ifndef DLMS_IGNORE_MALLOC
...
void svr_clear(dlmsServerSettings *settings) {
	cl_clear(&settings->base);
}

#ifndef DLMS_IGNORE_PLC
void plc_reset(dlmsSettings *settings) {
	...
	if (settings->interfaceType == DLMS_INTERFACE_TYPE_PLC) {
		if (settings->server) {
			settings->plcSettings.macSourceAddress =
					DLMS_PLC_SOURCE_ADDRESS_NEW;
			settings->plcSettings.macDestinationAddress =
					DLMS_PLC_SOURCE_ADDRESS_INITIATOR;
		} else {
			settings->plcSettings.macSourceAddress =
					DLMS_PLC_SOURCE_ADDRESS_INITIATOR;
			settings->plcSettings.macDestinationAddress =
...
void resetBlockIndex(dlmsSettings *settings) {
	settings->blockIndex = 1;
}

void resetFrameSequence(dlmsSettings *settings) {
	if (settings->server) {
...
unsigned char increaseReceiverSequence(unsigned char value) {
...
unsigned char increaseSendSequence(unsigned char value) {
	return (unsigned char) ((value & 0xF0) | ((value + 0x2) & 0xE));
}

#ifndef DLMS_IGNORE_HDLC_CHECK
unsigned char checkFrame(dlmsSettings *settings, unsigned char frame) {
	//If notify
	if (frame == 0x13) {
		return 1;
	}
	...
	if ((frame & 0x3) == 3) {
		if (frame == 0x93) {
			unsigned char isEcho = !settings->server && frame == 0x93
					&& (settings->senderFrame == 0x10
							|| settings->senderFrame == 0xfe)
					&& settings->receiverFrame == 0xE;
			resetFrameSequence(settings);
			return !isEcho;
		}
		if (frame == 0x73 && !settings->server) {
			return settings->senderFrame == 0xFE
					&& settings->receiverFrame == 0xE;
		}
		return 1;
	}
	...
	if ((frame & 0x1) == 1) {
		...
		settings->receiverFrame = increaseReceiverSequence(
				settings->receiverFrame);
		return 1;
	}
	//Handle I-frame.
	unsigned char expected;
	if ((settings->senderFrame & 0x1) == 0) {
		expected = increaseReceiverSequence(
				increaseSendSequence(settings->receiverFrame));
		if (frame == expected) {
		...
		if ((settings->receiverFrame & 0x10) == 0
				&& settings->windowSizeRX != 1) {
			expected = (unsigned char) (0x10
					| increaseSendSequence(settings->receiverFrame));
			if (frame == expected) {
				settings->receiverFrame = frame;
				return 1;
	...
	if ((!settings->server
			...
			|| (settings->server
					&& settings->receiverFrame
							== CLIENT_START_RCEIVER_FRAME_SEQUENCE)) {
		settings->receiverFrame = frame;
		return 1;
	}
...
unsigned char getNextSend(dlmsSettings *settings, unsigned char first) {
	if (first) {
		settings->senderFrame = increaseReceiverSequence(
				increaseSendSequence((unsigned char) settings->senderFrame));
	} else {
		settings->senderFrame = increaseSendSequence(
				(unsigned char) settings->senderFrame);
	}
	return (unsigned char) settings->senderFrame;
}

unsigned char getReceiverReady(dlmsSettings *settings) {
	settings->senderFrame = increaseReceiverSequence(
			(unsigned char) (settings->senderFrame | 1));
	return (unsigned char) (settings->senderFrame & 0xF1);
}

unsigned char getKeepAlive(dlmsSettings *settings) {
	settings->senderFrame = (unsigned char) (settings->senderFrame | 1);
	return (unsigned char) (settings->senderFrame & 0xF1);
}

#ifndef DLMS_IGNORE_HIGH_GMAC
unsigned char isCiphered(ciphering *cipher) {
...
void updateInvokeId(dlmsServerSettings *settings, unsigned char value) {
	if ((value & 0x80) != 0) {
